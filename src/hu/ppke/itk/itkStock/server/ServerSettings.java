...
/**
 * A settings singleton for the server.<br>
...
public enum ServerSettings {
	...
	/**
	 * Consturctor<br>
	 * Invokes load method to get the settings from the 'ServerConfig.xml' file<br>
	...
	/**
	 * This method loads the configuration file, if not present, generates one with default values.
	 */
	private void load()
	{
		try {			
			...
			System.err.println("File not found!\nGenerating default values to \'ServerConfig.xml\'");
			generateDefualt();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void reparseConfig()
	{
		...
		root.addContent(new Element("connection")
		    .addContent(new Element("port").setText(Integer.toString(dbPort))));
		root.addContent(new Element("db")
	...
	{
		Element root = new Element("server_config", "xsi", "http://www.w3.org/2001/XMLSchema-instance");
		root.setAttribute(new Attribute("noNamespaceSchemaLocation",
				"ServerConfig.xsd", Namespace.getNamespace("xsi", "http://www.w3.org/2001/XMLSchema-instance")));
	...
	private void makeFile(Element root)
	{
		...
		try {
			File f = new File("ServerConfig.xml");
	...
	public String toString()
	{
		...
		sb.append("port: ").append(Integer.toString(dbPort));
